...
with st.expander("🔧 Configurer les paliers", expanded=True):
    ...
    for i in range(5):  # 5 lignes modifiables
        cols = st.columns([1,1,1,1,1])
        with cols[0]: min_ = cols[0].number_input(f"Min_{i}", value=30000*i, min_value=10000, step=5000, label_visibility="collapsed")
...
# Annotations
for xi in range(10000, int(max_x)+1, pas_annotation):
